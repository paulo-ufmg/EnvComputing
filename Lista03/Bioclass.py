...
class Bioprof:
    ...
    def busca_cds(self,arg,codon_start):
        """Busca na squẽncias os segmentos codificantes de mRNA"""
        if(self.seq_existe(arg)):
            #indice = arg if isinstance(arg, int) else self.ids.index(arg) 
            if(self.get_tipo_seq(arg) != "PROTEINA"):
                stop = ['TAA','TAG','TGA'] if self.get_tipo_seq(arg) == "DNA" else ['UAA','UAG','UGA']
                seq = self.get_sequencia(arg)
                cds = []
                pos_i = 0
                finais = [0,0,0]
                print("Sequencia: ",seq)
                print("Códon: ",codon_start)
                print("localizado em:",seq.find(codon_start) )
                print("range: ",range(seq.count(codon_start)))

                for cd in range(seq.count(codon_start)):

                    pos_i = seq.find(codon_start,pos_i)
                    pos_f = pos_i + 3
                    finais = [seq.find(stop[0],pos_f),seq.find(stop[1],pos_f),seq.find(stop[2],pos_f)]
                    print("Finais: ",finais)
                    pos_f = min([n for n in finais if n>=0])
                    print("pos_i: ",pos_i)
                    print("pos_f: ",pos_f)
                    exit()

                    cds.append(seq[pos_i,pos_f])
                    print(cds)
                    exit()
